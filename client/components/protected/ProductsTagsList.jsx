...
import RegisterProductsTags from './RegisterProductsTags'
import { Route, BrowserRouter, Link, Redirect, Switch } from 'react-router-dom'
import ReactDom from 'react-dom';
import Popup from 'react-popup';

export default class ProductsTagsList extends Component {
	constructor() {
    super();
    this.state = {
      title: '',
  ...
  editItem(theId, theTitle) {
    const theRef = ref.child(`/productsTags/${theId}`);
  ...
  handleNewPartner(){
    ...
    content: <RegisterProductsTags />,
    className: 'alert',
    
});
  }
  componentDidMount() {
  	const theRef = ref.child(`productsTags`)
  ...
  removeItem(theId) {
    if(confirm("Do you really want to do this?")) {const theRef = ref.child(`/productsTags/${theId}`); theRef.remove()} 
      ...
      <div>
        ...
        <div className="col-xs-12 noPadding">
          <h3 className="row header smaller lighter blue">
            <span className="col-sm-4"><i className="fa fa-check-square-o bigger-160"></i>&nbsp; Product Tags&nbsp;&nbsp;<Link to="#" onClick={() => this.handleNewPartner()}><i className="fa  fa-user-plus green" aria-hidden="true"></i></Link></span> 
