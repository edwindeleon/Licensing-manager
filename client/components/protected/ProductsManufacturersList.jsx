...
import RegisterProductsManufacturers from './RegisterProductsManufacturers'
import { Route, BrowserRouter, Link, Redirect, Switch } from 'react-router-dom'
import ReactDom from 'react-dom';
import Popup from 'react-popup';

export default class ProductsManufacturersList extends Component {
	constructor() {
    super();
    this.state = {
      typeTitle: '',
  ...
  editItem(theId, theTitle) {
    const theRef = ref.child(`/productsManufacturers/${theId}`);

    let preTitle = this.state.typeTitle;
    let typeTitle = '';
    preTitle == '' ? typeTitle = theTitle : typeTitle = preTitle;
  ...
  handleNewPartner(){
    ...
    content: <RegisterProductsManufacturers />,
    className: 'alert',
  ...
  removeItem(theId) {
    if(confirm("Do you really want to do this?")) {const theRef = ref.child(`/productsManufacturers/${theId}`); theRef.remove()} 
      ...
      <div>
        ...
        <div className="col-xs-12 noPadding">
          <h3 className="row header smaller lighter blue">
            <span className="col-sm-4"><i className="fa fa-check-square-o bigger-160"></i>&nbsp; Manufacturers&nbsp;&nbsp;<Link to="#" onClick={() => this.handleNewPartner()}><i className="fa  fa-user-plus green" aria-hidden="true"></i></Link></span> 
        ...
        <table id="usersTables" className="table table-striped table-bordered" cellSpacing="0" width="100%">
          ...
          <tbody>
            {filteredEmails.map(entity => {
              let theId = entity.id;
              let theTitle = entity.typeTitle;
              return (
                <tr key={theId}>
                  <td><input ref={theId} disabled={this.state.myDisabled === false && this.state.myId == theId ? false : true} className="form-control" name="typeTitle" onChange={this.handleChange} defaultValue={entity.typeTitle} /></td>
