...
import RegisterLicenseTypes from './RegisterLicenseTypes'
import { Route, BrowserRouter, Link, Redirect, Switch } from 'react-router-dom'
import ReactDom from 'react-dom';
import Popup from 'react-popup';

export default class LicenseTypesList extends Component {
	constructor() {
    super();
    this.state = {
      title: '',
  ...
  editItem(theId, theTitle) {
    const theRef = ref.child(`/licenseTypes/${theId}`);
  ...
  handleNewPartner(){
    Popup.create({
    title: 'Create new License Type',
    content: <RegisterLicenseTypes />,
    className: 'alert',
    
});
  }
  componentDidMount() {
  	const theRef = ref.child(`licenseTypes`)
  ...
  removeItem(theId) {
    if(confirm("Do you really want to do this?")) {const theRef = ref.child(`/licenseTypes/${theId}`); theRef.remove()} 
      ...
      <div>
        ...
        <div className="col-xs-12 noPadding">
          <h3 className="row header smaller lighter blue">
            <span className="col-sm-4"><i className="fa fa-check-square-o bigger-160"></i>&nbsp; License Types&nbsp;&nbsp;<Link to="#" onClick={() => this.handleNewPartner()}><i className="fa  fa-user-plus green" aria-hidden="true"></i></Link></span> 
